...
const $$slug = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": `${program.title} — Nurul Imam Islamic School`, "description": program.desc }, { "default": ($$result2) => renderTemplate`  ${maybeRenderHead()}<div class="bg-white border-b px-6 py-3"> <div class="max-w-5xl mx-auto flex items-center gap-2 text-sm text-gray-500"> <a href="/" class="hover:text-green-600 transition">
Beranda
</a> <span>/</span> <a href="/#programs" class="hover:text-green-600 transition">
Program
</a> <span>/</span> <span class="text-gray-800 font-medium">${program.title}</span> </div> </div>  <section${addAttribute(`relative overflow-hidden bg-gradient-to-br py-20 px-6 ${style.bg}`, "class")}> <!-- Decorative circles --> <div class="pointer-events-none absolute -top-16 -right-16 h-64 w-64 rounded-full bg-white/10"></div> <div class="pointer-events-none absolute -bottom-10 -left-10 h-40 w-40 rounded-full bg-white/10"></div> <div class="relative max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-12"> <!-- Left: Text --> <div class="flex-1 flex flex-col items-center md:items-start text-center md:text-left gap-5"> <!-- Badge --> <span${addAttribute(`rounded-full px-4 py-1 text-xs font-semibold tracking-wide uppercase ${style.badge}`, "class")}>
Program Unggulan
</span> <!-- Icon + Title --> <div class="flex items-center gap-4"> <div${addAttribute(`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-2xl text-white shadow-md ${style.icon}`, "class")}> <i${addAttribute(`fa-solid ${program.icon}`, "class")}></i> </div> <h1 class="text-3xl md:text-4xl font-bold text-gray-800 leading-tight"> ${program.title} </h1> </div> <!-- Headline --> <p${addAttribute(`text-lg font-semibold ${style.text}`, "class")}> ${program.details.headline} </p> <!-- Desc — nada UMMI --> <p class="text-gray-600 text-base leading-relaxed max-w-xl">
Nak, di sini kami hadir dengan sepenuh hati untuk menemani perjalanan
          belajarmu. ${program.desc} Karena setiap anak adalah amanah terindah yang
...
Daftar Sekarang
<i class="fa-solid fa-arrow-right"></i> </a> </div> <!-- Right: Hero Image --> <div class="flex-1 w-full max-w-sm md:max-w-none"> <div class="relative rounded-3xl overflow-hidden shadow-2xl aspect-[4/3]"> <img${addAttribute(`https://picsum.photos/seed/${program.slug}/800/600`, "src")}${addAttribute(program.title, "alt")} class="w-full h-full object-cover"> <!-- Overlay badge --> <div class="absolute bottom-4 left-4 right-4"> <div class="bg-white/90 backdrop-blur-sm rounded-2xl px-4 py-3 flex items-center gap-3 shadow"> <div${addAttribute(`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white text-sm ${style.icon}`, "class")}> <i class="fa-solid fa-star"></i> </div> <div> <div class="text-xs text-gray-500">
...
</p> </div> </div> <!-- Highlights + Foto Kecil --> <div class="flex flex-col gap-4"> <!-- Mini photo --> <div class="rounded-2xl overflow-hidden shadow-sm aspect-video"> <img${addAttribute(`https://picsum.photos/seed/${program.slug}-2/400/250`, "src")} alt="Suasana program" class="w-full h-full object-cover"> </div> ${program.details.highlights.map((h) => renderTemplate`<div${addAttribute(`rounded-2xl border p-5 text-center bg-gradient-to-br shadow-sm ${style.bg} ${style.border}`, "class")}> <div${addAttribute(`text-2xl font-bold ${style.text}`, "class")}>${h.value}</div> <div class="mt-1 text-xs text-gray-500">${h.label}</div> </div>`)} </div> </div> </section>  <section class="py-12 px-6 bg-white"> <div class="max-w-3xl mx-auto text-center"> <div class="text-4xl mb-4">🤲</div> <blockquote class="text-lg md:text-xl text-gray-700 font-medium leading-relaxed italic">
