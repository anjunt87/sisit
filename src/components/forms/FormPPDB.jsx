// src/components/forms/FormPPDB.jsx
...
const INITIAL_FORM = {
  ...
  jenis_kelamin: "", asal_sekolah: "", nama_ayah: "",
  nama_ibu: "", no_hp: "", email: "", alamat: "",
};
...
/* -- inject Font Awesome CDN jika belum ada -- */
...
export default function FormPPDB() {
  ...
  // const jenjang = JENJANG_VALID.includes(j) ? j : "sd";
  // const meta = JENJANG_META[jenjang];

  const [jenjang, setJenjang] = useState("sd"); // default SSR aman
  ...
  const handleDrop = (name, e) => { e.preventDefault(); setDragOver(null); processFile(name, e.dataTransfer.files[0]); };
  const removeFile = name => { setFiles(p => ({ ...p, [name]: null })); setPreviews(p => ({ ...p, [name]: null })); };
  ...
  const back = to => { setStep(to); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) { setStatus("success"); setPesan(data.message || "Pendaftaran berhasil."); }
      else        { setStatus("error");   setPesan(data.error  || "Terjadi kesalahan."); }
    } catch { setStatus("error"); setPesan("Gagal terhubung ke server."); }
  };
  ...
  if (status === "success") {
    return (
      <div style={S.successPage}>
        {mounted && <style>{GLOBAL_CSS(meta)}</style>}
        <div style={S.successCard}>
          <div style={{ ...S.successIconWrap, background: meta.color }}>
            <i className="fas fa-check" style={{ color: "#fff", fontSize: 36 }} />
          </div>
          <h2 style={S.successTitle}>Pendaftaran Berhasil!</h2>
          <p style={S.successSub}>{pesanServer}</p>
          <div style={S.successInfo}>
            <div style={S.successRow}><span>Nama</span><strong>{form.nama_lengkap}</strong></div>
            <div style={S.successRow}><span>Jenjang</span><strong>{meta.nama}</strong></div>
          ...
          >
            <i className="fab fa-whatsapp" style={{ fontSize: 18 }} /> Konfirmasi via WhatsApp
          </a>
          <a href="/ppdb/ppdb-daftar" style={styles.backLink}>← Kembali ke Halaman PPDB</a>
  ...
  return (
    <div style={S.page}>
        ...
        <div style={S.modalOverlay} onClick={() => setModal(null)}>
          <div style={S.modalBox} onClick={e => e.stopPropagation()}>
            <div style={S.modalHeader}>
              <span style={S.modalTitle}>{previewModal.name}</span>
              <button style={S.modalClose} onClick={() => setModal(null)}><i className="fas fa-times" /></button>
            </div>
            {previewModal.type?.startsWith("image/")
              ? <img src={previewModal.url} alt="preview" style={S.modalImg} />
              : previewModal.type === "application/pdf"
                ? <iframe src={previewModal.url} style={S.modalFrame} title="PDF Preview" />
                : <div style={S.modalNoPreview}><i className="fas fa-file" style={{ fontSize: 40, display: "block", marginBottom: 12 }} />Preview tidak tersedia</div>}
          </div>
        </div>
      )}

      {/* ── HERO — solid color sesuai jenjang ── */}
      <div style={{ ...S.hero, background: meta.color }}>
        <div style={S.heroInner}>
          {/* brand bar atas */}
          {/* <div style={S.heroBrandRow}>
            <div style={S.heroBrandLogo}><i className="fas fa-mosque" style={{ fontSize: 18 }} /></div>
            <span style={S.heroBrandName}>Sekolah Islam Terpadu</span>
          </div> */}

          {/* konten utama — icon + teks */}
          <div style={S.heroBody}>
            <div style={{ ...S.heroIconCircle }}>
              <i className={`fas ${meta.faIcon}`} style={{ fontSize: 36, color: meta.color }} />
            </div>
            <div>
              <div style={S.heroBadge}>PPDB 2026</div>
              <h1 style={S.heroTitle}>Formulir Pendaftaran</h1>
              <p style={S.heroSub}>
                <i className="fas fa-tag" style={{ marginRight: 5 }} />{meta.nama}
                <span style={S.heroDivider} />
                <i className="fas fa-child" style={{ marginRight: 5 }} />Usia {meta.usia}
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ lineHeight: 0 }}>
          <svg viewBox="0 0 1440 56" fill="none" xmlns="http://www.w3.org/2000/svg"
            preserveAspectRatio="none" style={{ width: "100%", height: 56, display: "block" }}>
            <path d="M0,28 C480,70 960,0 1440,28 L1440,56 L0,56 Z" fill="#f5f6fa" />
          </svg>
        </div>
      </div>
          ...
          <div key={s.no} style={S.stepItem}>
            <div style={{
              ...
              {step > s.no
                ...
                : <i className={`fas ${s.fa}`} style={{ fontSize: 13 }} />}
            </div>
            <span style={{ ...S.stepLabel, color: step >= s.no ? meta.accent : "#94a3b8", fontWeight: step === s.no ? 700 : 500 }}>
              {s.label}
            </span>
            {i < STEPS.length - 1 && (
              <div style={{ ...S.stepLine, background: step > s.no ? meta.color : "#e2e8f0" }} />
            )}
          </div>
        ))}
      </div>
      ...
      <div style={S.container}>
        <div style={S.card}>
          ...
          {step === 1 && (
            <div className="fade-in">
              <SectionTitle fa="fa-user-circle" title="Data Diri Siswa" desc="Isi informasi lengkap calon siswa" accent={meta.accent} />
              <Field label="Nama Lengkap" error={errors.nama_lengkap} required>
                <FaInput name="nama_lengkap" value={form.nama_lengkap} onChange={handleChange}
                  placeholder="Contoh: Muhammad Rizki Al-Farisi" accent={meta.accent} error={errors.nama_lengkap} fa="fa-id-card" />
              </Field>
              <div style={S.grid2}>
                <Field label="Tempat Lahir">
                  <FaInput name="tempat_lahir" value={form.tempat_lahir} onChange={handleChange}
                    placeholder="Jakarta" accent={meta.accent} fa="fa-map-marker-alt" />
                </Field>
                <Field label="Tanggal Lahir" error={errors.tanggal_lahir} required>
                  <FaInput type="date" name="tanggal_lahir" value={form.tanggal_lahir} onChange={handleChange}
                    accent={meta.accent} error={errors.tanggal_lahir} fa="fa-calendar-alt" />
                </Field>
              </div>
              <Field label="Jenis Kelamin" error={errors.jenis_kelamin} required>
                <div style={S.radioGroup}>
                  {[{ v: "L", l: "Laki-laki", fa: "fa-mars" }, { v: "P", l: "Perempuan", fa: "fa-venus" }].map(({ v, l, fa }) => (
                    <label key={v} style={{ ...S.radioCard, borderColor: form.jenis_kelamin === v ? meta.color : "#e2e8f0", background: form.jenis_kelamin === v ? meta.light : "#fff" }}>
                      <input type="radio" name="jenis_kelamin" value={v} checked={form.jenis_kelamin === v} onChange={handleChange} style={{ display: "none" }} />
                      <i className={`fas ${fa}`} style={{ color: form.jenis_kelamin === v ? meta.accent : "#94a3b8", fontSize: 20, marginBottom: 4 }} />
                      <span style={{ color: form.jenis_kelamin === v ? meta.accent : "#64748b", fontWeight: 700, fontSize: 13 }}>{l}</span>
                    </label>
                  ))}
                </div>
                {errors.jenis_kelamin && <ErrMsg msg={errors.jenis_kelamin} />}
              </Field>
              <Field label="Asal Sekolah / TK Sebelumnya">
                <FaInput name="asal_sekolah" value={form.asal_sekolah} onChange={handleChange}
                  placeholder="(Opsional)" accent={meta.accent} fa="fa-school" />
              </Field>
              <button className="btn-primary" style={{ ...S.btnPrimary, background: meta.color, marginTop: 8 }} onClick={() => goNext(validateStep1, 2)}>
                Lanjut ke Data Orang Tua <i className="fas fa-arrow-right" style={{ marginLeft: 8 }} />
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="fade-in">
              <SectionTitle fa="fa-users" title="Data Orang Tua & Kontak" desc="Informasi wali yang dapat dihubungi" accent={meta.accent} />
              <div style={S.grid2}>
                <Field label="Nama Ayah">
                  <FaInput name="nama_ayah" value={form.nama_ayah} onChange={handleChange} placeholder="Nama lengkap ayah" accent={meta.accent} fa="fa-male" />
                </Field>
                <Field label="Nama Ibu">
                  <FaInput name="nama_ibu" value={form.nama_ibu} onChange={handleChange} placeholder="Nama lengkap ibu" accent={meta.accent} fa="fa-female" />
                </Field>
              </div>
              <Field label="Nomor HP / WhatsApp" error={errors.no_hp} required>
                <FaInput name="no_hp" value={form.no_hp} onChange={handleChange} placeholder="08xx-xxxx-xxxx" accent={meta.accent} error={errors.no_hp} fa="fa-phone" />
              </Field>
              <Field label="Alamat Email" error={errors.email}>
                <FaInput name="email" value={form.email} onChange={handleChange} placeholder="email@contoh.com (opsional)" accent={meta.accent} error={errors.email} fa="fa-envelope" />
              </Field>
              <Field label="Alamat Lengkap">
                <div style={{ position: "relative" }}>
                  <i className="fas fa-map-marked-alt" style={{ position: "absolute", left: 14, top: 14, color: "#94a3b8", fontSize: 14 }} />
              ...
              <div style={S.btnRow}>
                <BtnBack onClick={() => back(1)} />
                <button className="btn-primary" style={{ ...S.btnPrimary, background: meta.color }} onClick={() => goNext(validateStep2, 3)}>
                  Lanjut ke Dokumen <i className="fas fa-arrow-right" style={{ marginLeft: 8 }} />
                </button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="fade-in">
              <SectionTitle fa="fa-folder-open" title="Upload Dokumen" desc="Format: JPG, PNG, PDF · Maks. 5MB per file" accent={meta.accent} />
              {[
                ...
                <DropZone key={d.name} label={d.label} name={d.name} required={d.required}
                  file={files[d.name]} preview={previews[d.name]} error={errors[d.name]}
                  onFile={processFile} onDrop={handleDrop} onRemove={removeFile}
                  onPreview={setModal} dragOver={dragOver} setDragOver={setDragOver}
                  accent={meta.accent} color={meta.color} light={meta.light} onChange={handleFile} />
              ))}
              <div style={S.btnRow}>
                <BtnBack onClick={() => back(2)} />
                <button className="btn-primary" style={{ ...S.btnPrimary, background: meta.color }} onClick={() => goNext(validateStep3, 4)}>
                  Lanjut ke Konfirmasi <i className="fas fa-arrow-right" style={{ marginLeft: 8 }} />
                </button>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="fade-in">
              <SectionTitle fa="fa-clipboard-check" title="Konfirmasi Pendaftaran" desc="Periksa kembali data sebelum mengirim" accent={meta.accent} />
              <div style={S.summaryCard}>
                <SummaryGroup title="Data Siswa" fa="fa-user" accent={meta.accent} color={meta.color} light={meta.light} items={[
                ...
                <SummaryGroup title="Data Orang Tua" fa="fa-users" accent={meta.accent} color={meta.color} light={meta.light} items={[
                  { label: "Nama Ayah", val: form.nama_ayah },
                  { label: "Nama Ibu",  val: form.nama_ibu  },
                  { label: "No HP",     val: form.no_hp     },
                  { label: "Email",     val: form.email     },
                ]} />
                <SummaryGroup title="Dokumen" fa="fa-paperclip" accent={meta.accent} color={meta.color} light={meta.light} items={[
              ...
              <div style={{ ...S.jenjangBadge, background: meta.light, borderColor: meta.color, color: meta.accent }}>
                <i className={`fas ${meta.faIcon}`} style={{ marginRight: 8 }} />
                Mendaftar untuk jenjang <strong style={{ marginLeft: 4 }}>{meta.nama}</strong>
              </div>
              {status === "error" && (
                <div style={S.errorBox}><i className="fas fa-exclamation-triangle" style={{ marginRight: 8 }} />{pesanServer}</div>
              )}
              <div style={S.btnRow}>
                <BtnBack onClick={() => back(3)} />
                <button className="btn-primary"
                  style={{ ...S.btnPrimary, background: meta.color, opacity: status === "loading" ? 0.75 : 1 }}
                  onClick={handleSubmit} disabled={status === "loading"}>
                  {status === "loading"
                    ? <><i className="fas fa-spinner fa-spin" style={{ marginRight: 8 }} />Mengirim...</>
                    : <><i className="fas fa-paper-plane" style={{ marginRight: 8 }} />Kirim Pendaftaran</>}
                </button>
              </div>
            </div>
          )}
        ...
        <div style={S.footer}>© 2026 PPDB Sekolah Islam Terpadu · Pertanyaan? Hubungi kami</div>
      </div>
    </div>
  );
}
...
function DropZone({ label, name, required, file, preview, error, onFile, onDrop, onRemove, onPreview, dragOver, setDragOver, accent, color, light, onChange }) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={S.fieldLabel}>{label}{required && <span style={{ color: "#ef4444", marginLeft: 3 }}>*</span>}</div>
      {!file ? (
        <div
        ...
        >
          <input ref={inputRef} type="file" name={name} accept="image/*,application/pdf" onChange={onChange} style={{ display: "none" }} />
          <i className={`fas ${isDragging ? "fa-folder-open" : "fa-cloud-upload-alt"}`}
            style={{ fontSize: 34, color: isDragging ? color : "#94a3b8", marginBottom: 10 }} />
          <div style={S.dropText}>{isDragging ? "Lepaskan file di sini" : "Klik atau seret file ke sini"}</div>
          <div style={S.dropHint}>JPG, PNG, atau PDF · Maks. 5MB</div>
        </div>
      ) : (
        <div style={{ ...S.fileCard, borderColor: color, background: light }}>
          {preview?.type?.startsWith("image/") && (
            <img src={preview.url} alt="" style={S.fileThumb} onClick={() => onPreview(preview)} />
          )}
          {preview?.type === "application/pdf" && (
            <div style={{ ...S.fileThumb, display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", borderRadius: 10, cursor: "pointer" }} onClick={() => onPreview(preview)}>
              <i className="fas fa-file-pdf" style={{ fontSize: 30, color: "#ef4444" }} />
            </div>
          )}
          {!preview?.type?.startsWith("image/") && preview?.type !== "application/pdf" && (
            <div style={{ ...S.fileThumb, display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", borderRadius: 10 }}>
              <i className="fas fa-file" style={{ fontSize: 28, color: accent }} />
            </div>
          )}
          <div style={S.fileInfo}>
            <div style={{ ...S.fileName, color: accent }}><i className="fas fa-check-circle" style={{ marginRight: 6 }} />{file.name}</div>
            <div style={S.fileSize}>{(file.size / 1024).toFixed(1)} KB</div>
            <div style={S.fileActions}>
              <button style={{ ...S.fileBtn, color: accent, borderColor: accent }} onClick={() => onPreview(preview)}>
                <i className="fas fa-eye" style={{ marginRight: 4 }} />Preview
              </button>
              <button style={{ ...S.fileBtn, color: "#ef4444", borderColor: "#ef4444" }} onClick={() => onRemove(name)}>
                <i className="fas fa-trash-alt" style={{ marginRight: 4 }} />Hapus
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <h3 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 800, color: "#1e293b", display: "flex", alignItems: "center", gap: 10 }}>
        <i className={`fas ${fa}`} style={{ color: accent, fontSize: 18 }} />{title}
      </h3>
      <p style={{ margin: 0, fontSize: 13, color: "#94a3b8" }}>{desc}</p>
    </div>
  );
}
...
function ErrMsg({ msg }) {
  return <div style={S.errMsg}><i className="fas fa-exclamation-circle" style={{ marginRight: 4 }} />{msg}</div>;
}
...
function FaInput({ name, value, onChange, placeholder, type = "text", accent, error, fa }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <i className={`fas ${fa}`} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: focused ? accent : "#94a3b8", fontSize: 14, pointerEvents: "none", transition: "color 0.2s" }} />
      <input
...
function SummaryGroup({ title, fa, items, accent, color, light }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, padding: "7px 12px", background: light, borderRadius: 10 }}>
        <i className={`fas ${fa}`} style={{ color: accent, fontSize: 12 }} />
        <span style={{ fontWeight: 800, fontSize: 11, color: accent, letterSpacing: 0.5, textTransform: "uppercase" }}>{title}</span>
      </div>
      {items.map(({ label, val, ok, optional }) => (
        <div key={label} style={S.summaryRow}>
          <span style={S.summaryLabel}>{label}</span>
          <span style={{ ...S.summaryVal, color: ok === false && !optional ? "#ef4444" : ok === false && optional ? "#94a3b8" : "#1e293b" }}>
            {ok === true  && <i className="fas fa-check-circle"  style={{ color, marginRight: 5 }} />}
            {ok === false && !optional && <i className="fas fa-times-circle" style={{ color: "#ef4444", marginRight: 5 }} />}
            {val || "—"}
          </span>
        </div>
      ))}
...
function BtnBack({ onClick }) {
  return (
    <button style={S.btnBack} onClick={onClick}>
      <i className="fas fa-arrow-left" style={{ marginRight: 8 }} />Kembali
    </button>
  );
}
...
const S = {
 ...
 heroIconCircle:{ width: 80, height: 80, borderRadius: 22, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 8px 24px rgba(0,0,0,.14)" },
  ...
  radioCard:    { flex: 1, borderWidth: "2px",
  borderStyle: "solid",
  borderColor: "#e2e8f0", borderRadius: 14, padding: "14px 16px", cursor: "pointer", textAlign: "center", transition: "all 0.2s", userSelect: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 },
  ...
  fileCard:     { display: "flex", gap: 14, padding: 14, borderRadius: 16, borderWidth: "2px",
  borderStyle: "solid",
  ...
  fileBtn:      { background: "transparent", borderWidth: "1.5px",
  borderStyle: "solid",
  borderColor: "#e2e8f0", borderRadius: 8, padding: "4px 10px", fontSize: 12, cursor: "pointer", fontWeight: 700, fontFamily: "inherit" },
